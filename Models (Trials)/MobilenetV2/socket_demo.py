...
s.send(("sgrge864358ewr").encode(encoding='utf-8'))
receive = s.recv(1024).decode('utf-8')
while(not len(receive)):
    receive = s.recv(1024).decode('utf-8')
while (receive) != "sgrge864358ewr":
    while(not len(receive)):
        receive = s.recv(1024).decode('utf-8')
...
while cap.isOpened():
    ...
    for (x, y, w, h) in faces:
        ...
        s.send((emotion_dict[int(np.argmax(prediction2))] + "\0"
                ).encode(encoding='utf-8'))
        cv2.putText(frame, emotion_dict[int(np.argmax(
            prediction2))], (x, y), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
        # print('hi')
