...
model = load_model("./whole_model.hdf5" , compile=False)
...
def show_faces(img, boxes, landmarks):
    ...
    for bb, ll in zip(boxes, landmarks):
        ...
        cv2.putText(img, emotion_dict[int(np.argmax(emotion))] + "\n" + sleep_dict[int(np.argmax(sleep))], (x1, y1), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
        for j in range(5):
